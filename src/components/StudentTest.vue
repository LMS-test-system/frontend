            ...
            <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="test in tests" :key="test.id" class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                
                                <td scope="row" class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{test.name}}
                                </td>
                                <td class="px-4 py-2">
                                    <span class="bg-primary-100  text-primary-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-primary-900 dark:text-primary-300">{{test.subject.name}}</span>
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{test.type}}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{test.time_limit}} min</td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white"><Button @click="startTest(test.id)" color="green">Boshlash</Button></td>
