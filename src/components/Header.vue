<template>
    ...
    <div  class="container mx-auto">
        <div v-if="user" class="h-[40px] lg:h-[65px] border-b border-[#DCDCDC] flex justify-between px-3 " >
            ...
            <div class="flex items-center text-[22px] text-gray-500 gap-4 relative">
              ...
              <Avatar @click="toggleSettings" status="online" size="sm" rounded :img="`${api_url}/image/file/${user.image?.file_name}`" />
                <div class="w-[225px] h-[250px] rounded-t-md absolute setting top-12 right-0 z-10" :class="isSettings ? '' : 'hidden'">
                    ...
                    <div class="flex flex-col gap-1 pt-10 items-center justify-center text-black text-[14px]">
                        ...
                        <div class="w-full flex flex-col gap-3 mt-2 px-3">
                            <button @click="router.push({ name: 'account', params: {id: user.id} })">
